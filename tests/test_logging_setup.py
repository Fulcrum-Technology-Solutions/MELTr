...
def test_setup_logging_env_log_file_overrides_config(tmp_path, monkeypatch):
    """LOGFORGE_LOG_FILE wins over config.logging.file."""
    monkeypatch.delenv("LOGFORGE_LOG_FILE", raising=False)
    home = tmp_path / "home"
    home.mkdir()
    override = tmp_path / "from_env.log"
    cfg_path = tmp_path / "from_config.log"
    monkeypatch.setenv("LOGFORGE_LOG_FILE", str(override))

    cfg = create_default_config(home)
    cfg = cfg.model_copy(update={"logging": cfg.logging.model_copy(update={"file": str(cfg_path)})})
    setup_logging(cfg)

    root = logging.getLogger("logforge")
...
def test_setup_logging_default_uses_default_application_log_file(tmp_path, monkeypatch):
    ...
    home = tmp_path / "home"
    home.mkdir()
    expect_log = (tmp_path / "opt" / "bundle" / "logs" / "logforge.log").resolve()

    cfg = create_default_config(home)
    cfg = cfg.model_copy(update={"logging": cfg.logging.model_copy(update={"file": None})})

    with patch("logforge.utils.logging.default_application_log_file", return_value=expect_log):
        setup_logging(cfg)
