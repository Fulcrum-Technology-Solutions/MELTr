...
def test_restart_uses_systemd_when_unit_exists_and_restart_ok(monkeypatch):
    ...
    r.restart(
        timeout=30, foreground=False, console=Console(), api_stop=api_stop, api_start=api_start
    )
...
def test_restart_falls_back_when_unit_missing(monkeypatch):
    ...
    r.restart(
        timeout=12, foreground=True, console=Console(), api_stop=api_stop, api_start=api_start
    )
...
def test_restart_falls_back_when_systemd_restart_fails(monkeypatch):
    ...
    r.restart(
        timeout=9, foreground=False, console=Console(), api_stop=api_stop, api_start=api_start
    )
