...
from logforge.core.paths import default_application_log_file
...
def setup_logging(config: Optional[Config] = None, log_level: Optional[str] = None) -> None:
    ...
    1. ``LOGFORGE_LOG_FILE`` environment variable (systemd/operator override)
    2. ``config.logging.file`` when non-empty
    3. :func:`~logforge.core.paths.default_application_log_file` (bundle: ``<install>/logs/…``)
    ...
    """
    raw_env_log = os.getenv("LOGFORGE_LOG_FILE")
