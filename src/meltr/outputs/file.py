...
from meltr.core.config import OutputDefinition, OutputRotationConfig
from meltr.outputs.base import OutputHandler
from meltr.outputs.path_resolver import PathTemplateContext, resolve_path_template
...
class FileOutputHandler(OutputHandler):
    ...
    def __init__(
        ...
        name: str,
        path: str,
        rotation: OutputRotationConfig | None = None,
    ) -> None:
        ...
        self._path_context: PathTemplateContext | None = None
    ...
    def _resolve_path(self, force_refresh: bool = False) -> Path:
        ...
        needs_refresh = (
            force_refresh
            or self._resolved_path_cache is None
            or self._last_path_resolution_time == 0
            or
            # Re-resolve if template contains time variables and enough time has passed
            (
                "{date}" in self.path_template
                or "{hour}" in self.path_template
                or "{timestamp}" in self.path_template
            )
        ...
        if not self._path_context:
            logger.warning(
                f"FileOutputHandler '{self.name}': No template context set, using fallback resolution"
            )
    ...
    def _parse_time_interval(self, interval: str | None) -> int | None:
    ...
    def _parse_size(self, size: str | None) -> int:
