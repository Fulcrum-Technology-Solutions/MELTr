...
class OutputHandler(ABC):
    ...
    def _handle_write_error(self, event: str, error: Exception) -> None:
        ...
        with self._retry_lock:
            ...
            if self.retry_config and self.retry_config.max_attempts != 0:
                if (
                    self.retry_config.max_attempts > 0
                    and self._retry_attempt >= self.retry_config.max_attempts
                ):
                    logger.error(f"Output handler {self.name}: Max retry attempts reached")
                    return
    ...
    def initialize(self) -> None:  # noqa: B027 — optional hook, not abstract
