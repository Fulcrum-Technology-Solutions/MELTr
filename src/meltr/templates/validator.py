...
def _validate_metadata_structure(
    metadata: TemplateMetadata, metadata_path: Path, result: ValidationResult
) -> None:
    ...
    if vendor_from_path and metadata.vendor != vendor_from_path:
        result.add_warning(
            f"Metadata vendor '{metadata.vendor}' doesn't match directory '{vendor_from_path}'"
        )

    if product_from_path and metadata.product != product_from_path:
        result.add_warning(
            f"Metadata product '{metadata.product}' doesn't match directory '{product_from_path}'"
        )

    if data_source_from_path and metadata.data_source != data_source_from_path:
        result.add_warning(
            f"Metadata data_source '{metadata.data_source}' doesn't match directory '{data_source_from_path}'"
        )
