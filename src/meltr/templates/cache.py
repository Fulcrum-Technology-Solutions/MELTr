...
from meltr.templates.loader import TemplateInfo, TemplateLoader
...
class CachedTemplate:
    ...
    def __init__(self, template_info: TemplateInfo) -> None:
        ...
        self.template_mtime = (
            template_info.template_path.stat().st_mtime
            if template_info.template_path.exists()
            else 0
        )
        self.metadata_mtime = (
            template_info.metadata_path.stat().st_mtime
            if template_info.metadata_path.exists()
            else 0
        )
    ...
    def is_stale(self) -> bool:
        ...
        current_metadata_mtime = (
            self.template_info.metadata_path.stat().st_mtime
            if self.template_info.metadata_path.exists()
            else 0
        )

        return (
            current_template_mtime > self.template_mtime
            or current_metadata_mtime > self.metadata_mtime
        )
