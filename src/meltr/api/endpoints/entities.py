...
@router.get("")
async def get_entities_summary(registry: Annotated[EntityRegistry, Depends(get_registry)]) -> dict:
...
async def import_entities(
    ...
    if merge:
        ...
        merged_data["services"] = current_data.get("services", []) + entities_data.get(
            "services", []
        )
        ...
        if "network_ranges" in entities_data:
            merged_data["network_ranges"] = current_data.get(
                "network_ranges", []
            ) + entities_data.get("network_ranges", [])
...
@router.post("/reload")
async def reload_entities(registry: Annotated[EntityRegistry, Depends(get_registry)]) -> dict:
