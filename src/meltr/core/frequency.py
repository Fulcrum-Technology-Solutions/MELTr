...
from meltr.core.config import FrequencyConfig, FrequencyVariation
...
def calculate_rate_from_template_metadata(metadata: TemplateMetadata, timezone: str = 'UTC') -> float:
    ...
    if "business_hours" in metadata.time_patterns:
        if current_day in [1, 2, 3, 4, 5]:  # Mon-Fri
            if time(9, 0) <= current_time <= time(17, 0):
                multiplier = metadata.business_hours_multiplier if metadata.business_hours_multiplier is not None else 1.0
                return base_rate * multiplier
    ...
    if "weekend" in metadata.time_patterns:
        if current_day in [6, 7]:  # Sat-Sun
            multiplier = metadata.weekend_multiplier if metadata.weekend_multiplier is not None else 1.0
            return base_rate * multiplier
    ...
    if "night_hours" in metadata.time_patterns:
        if current_day in [1, 2, 3, 4, 5]:  # Mon-Fri
            ...
            if current_time >= time(17, 0) or current_time < time(9, 0):
                multiplier = metadata.night_hours_multiplier if metadata.night_hours_multiplier is not None else 1.0
                return base_rate * multiplier
