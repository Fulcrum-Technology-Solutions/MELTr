...
from meltr.cli.user_utils import (
    check_root,
    ensure_service_user_and_group,
    ...
    if create_user:
        service_uid, service_gid = ensure_service_user_and_group(
            ...
            on_no_pwd_grp=lambda: console.print(
                "[yellow]⚠ pwd/grp not available, skipping user/group creation[/yellow]"
            ),
...
def _interactive_wizard(home: Path) -> "Config":
    ...
    # Organization info
    Prompt.ask("Organization name", default="Acme Corporation")
    Prompt.ask("Organization domain", default="acme.com")
    ...
    if install_starter:
        console.print(
            "[yellow]Note: Template installation will be implemented in template commands[/yellow]"
        )
...
def _load_bundled_sample_entities() -> dict | None:
