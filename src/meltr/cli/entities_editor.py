...
def _edit_organization(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if Confirm.ask("Configure additional organization details?", default=False):
        ...
        if Confirm.ask("Configure contacts?", default=False):
            ...
            contacts["security"] = Prompt.ask(
                "Security contact email", default=contacts.get("security", "")
            )
            contacts["it_support"] = Prompt.ask(
                "IT support email", default=contacts.get("it_support", "")
            )
            contacts["helpdesk"] = Prompt.ask(
                "Helpdesk email", default=contacts.get("helpdesk", "")
            )
...
def _create_user_interactive(existing_users: list[dict[str, Any]]) -> dict[str, Any] | None:
...
def _remove_user_interactive(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if Confirm.ask(f"\n[yellow]Remove user '{user.get('username', '')}'?", default=False):
        users.pop(choice - 1)
        console.print("[green]✓ Removed user[/green]")
...
def _create_device_interactive(existing_devices: list[dict[str, Any]]) -> dict[str, Any] | None:
    ...
    mac_address = Prompt.ask(
        "MAC address (required, format: XX:XX:XX:XX:XX:XX)", default="00:00:00:00:00:00"
    )
...
def _remove_device_interactive(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if Confirm.ask(f"\n[yellow]Remove device '{device.get('hostname', '')}'?", default=False):
        ...
        console.print("[green]✓ Removed device[/green]")
...
def _create_service_interactive(existing_services: list[dict[str, Any]]) -> dict[str, Any] | None:
...
def _edit_service_interactive(data: dict[str, Any]) -> dict[str, Any]:
    ...
    for i, service in enumerate(services, 1):
        console.print(
            f"  [{i}] {service.get('name', '')} - {service.get('protocol', '')}/{service.get('port', '')}"
        )
...
def _remove_service_interactive(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if Confirm.ask(f"\n[yellow]Remove service '{service.get('name', '')}'?", default=False):
        ...
        console.print("[green]✓ Removed service[/green]")
