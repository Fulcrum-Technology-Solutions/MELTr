...
@app.command("show")
def config_show(
    path: str | None = typer.Option(None, "--path", help="Show specific config section"),
    ...
    try:
        ...
        console.print("[red]Error: Config file not found. Run 'meltr init' first.[/red]")
        raise typer.Exit(code=1)
...
def config_validate(
    ...
    try:
        ...
        console.print("[yellow]⚠ Config file not found. Run 'meltr init' to create one.[/yellow]")
        raise typer.Exit(code=1)
    except Exception as e:
...
def _quick_add_generator(
    ...
    try:
        ...
        if template not in templates:
            console.print(
                f"[yellow]Warning: Template '{template}' not found. Will be validated on start.[/yellow]"
            )
    except Exception as e:
    ...
    if not outputs_str:
        ...
        console.print(
            f"[yellow]No outputs specified, using first output: {selected_outputs[0]}[/yellow]"
        )
    else:
...
def _quick_edit_generator(
    ...
    if not changes:
        console.print(
            "[yellow]No changes specified. Use --enable/--disable, --outputs, or --timezone[/yellow]"
        )
        return
...
def config_edit(
    section: str | None = typer.Option(
        None, "--section", help="Edit specific section (outputs, generators, api, engine, logging)"
    ),
    new: bool = typer.Option(
        False, "--new", help="Start with fresh default config instead of loading existing"
    ),
    add_generator: str | None = typer.Option(
        None,
        "--add-generator",
        "--add",
        help="Quick-add generator: template ID (e.g., 'microsoft/azure/signin')",
    ),
    generator_name: str | None = typer.Option(
        None, "--name", help="Generator name (used with --add-generator or --edit-generator)"
    ),
    outputs: str | None = typer.Option(
        None,
        "--outputs",
        help="Comma-separated output names (used with --add-generator or --edit-generator)",
    ),
    edit_generator: str | None = typer.Option(
        None, "--edit-generator", "--edit-gen", help="Quick-edit generator by name"
    ),
    enable: bool | None = typer.Option(
        None, "--enable/--disable", help="Enable/disable generator (used with --edit-generator)"
    ),
    timezone: str | None = typer.Option(
        None, "--timezone", help="Timezone override (used with --add-generator or --edit-generator)"
    ),
) -> None:
