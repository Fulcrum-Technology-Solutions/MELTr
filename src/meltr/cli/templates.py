...
def templates_validate(
    ...
    if template_path is None:
        ...
        console.print("[dim]Example: meltr templates validate --path /path/to/template/directory[/dim]")
        raise typer.Exit(code=1)
        ...
        if vendor:
            ...
            try:
                ...
                for vendor_data in vendors:
                    ...
                    console.print(f"  [dim]Use: meltr templates browse --vendor {vendor_id}[/dim]\n")
            except Exception as e:
...
def _do_install_template(
    ...
    try:
        ...
        def emit_template_installed_events(*, vendor_id: str, product_id: Optional[str], method: str) -> None:
            ...
            """
            try:
                from meltr.telemetry import TelemetryClient, TelemetryEvent, get_actor_id, telemetry_enabled
        ...
        if local_file:
            ...
            from meltr.community.package import extract_forge_package, validate_package_structure, install_package
            import tempfile
        ...
        if vendor or len(parts) == 1:
            ...
            try:
                ...
                console.print(f"[dim]Use 'meltr templates browse --vendor {vendor_id}' to see available products[/dim]")
                raise typer.Exit(code=1)
            ...
            if len(parts) >= 2:
                vendor_id = parts[0]
                product_id = parts[1]
                console.print(f"[cyan]  Product: meltr templates install {vendor_id}/{product_id} --product[/cyan]")
                console.print(f"[cyan]  Vendor:  meltr templates install {vendor_id} --vendor[/cyan]")
            else:
                console.print(f"[cyan]  Vendor:  meltr templates install {parts[0]} --vendor[/cyan]")
...
def templates_install(
    ...
    try:
        ...
        if list_vendors:
            try:
                ...
                for vendor_data in vendors:
                    ...
                    console.print(f"  [dim]Install: meltr templates install {vendor_id} --vendor[/dim]\n")
        ...
        if not vendor_or_product:
            ...
            console.print("[dim]Hint: Use 'meltr templates install --list-vendors' to see available vendors[/dim]")
            console.print("[dim]Examples:[/dim]")
            console.print("[dim]  Vendor:  meltr templates install microsoft --vendor[/dim]")
            console.print("[dim]  Product: meltr templates install microsoft/windows --product[/dim]")
            raise typer.Exit(code=1)
...
def templates_compare(
    ...
    try:
        ...
        if only_remote:
            ...
            console.print(f"  [dim]Use 'meltr templates search' or 'meltr templates browse' to discover them[/dim]")
