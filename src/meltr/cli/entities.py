...
from meltr.core.paths import get_entities_path
...
@app.command("list")
def entities_list(
    entity_type: Literal["users", "devices", "services"] | None = typer.Option(
    ...
    try:
        if entity_type:
            ...
            console.print(
                f"Organization: {data['organization']['name']} ({data['organization']['domain']})"
            )
        ...
        if entity_type == "user":
            entity = next(
                (
                    ...
                    if e.get("username", "").lower() == identifier.lower()
                ),
                None,
            )
        elif entity_type == "device":
            entity = next(
                (e for e in data["entities"] if e.get("hostname", "") == identifier), None
            )
...
def entities_validate(
    entities_path: Path | None = typer.Option(
        None,
        "--file",
        "-f",
...
def entities_import(
    ...
    merge: bool = typer.Option(
        False, "--merge", help="Merge with existing entities instead of replacing"
    ),
    api_url: str | None = typer.Option(None, "--api-url", envvar="MELTR_API_URL"),
    api_key: str | None = typer.Option(None, "--api-key", envvar="MELTR_API_KEY"),
) -> None:
    ...
    if is_healthy:
        ...
        try:
            response = client.post(
                "/api/entities/import", json=import_data, params={"merge": merge}
            )
            response.raise_for_status()
            data = response.json()
    ...
    console.print("[dim]Note: Restart service or use 'meltr entities reload' to load changes[/dim]")
    ...
    if merge and existing_data:
        ...
        merged_data["services"] = existing_data.get("services", []) + import_data.get(
            "services", []
        )
    ...
    console.print("[green]✓ Entities imported successfully[/green]")
...
def entities_export(
    ...
    try:
        ...
        console.print("[red]Error: Entities file not found. Run 'meltr init' first.[/red]")
        raise typer.Exit(code=1)
    except Exception as e:
        console.print(f"[red]Error: {e}[/red]")
        raise typer.Exit(code=1)
...
@app.command("add")
def entities_add(
    entity_type: Literal["user", "device", "service"] = typer.Argument(
        ..., help="Type of entity to add"
    ),
) -> None:
    """Interactively add a new entity."""
    from meltr.cli.entities_editor import (
    ...
    try:
        ...
        from meltr.cli.entities_editor import _create_default_entities_structure
...
def entities_edit() -> None:
    ...
    from meltr.cli.entities_editor import entities_editor
